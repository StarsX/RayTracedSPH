...
void RayTracedSPH::LoadPipeline()
{
...
void RayTracedSPH::LoadAssets()
{
	...
	XUSG_N_RETURN(m_commandListEZ->Create(commandList.get(), 1, 32), ThrowIfFailed(E_FAIL));
	...
	m_commandQueue->ExecuteCommandList(m_commandListEZ->AsCommandList());
...
void RayTracedSPH::OnKeyUp(uint8_t key)
{
	switch (key)
	{
	case VK_SPACE:
...
void RayTracedSPH::OnLButtonDown(float posX, float posY)
{
	m_tracking = true;
	m_mousePt = XMFLOAT2(posX, posY);
}

void RayTracedSPH::OnLButtonUp(float posX, float posY)
{
	m_tracking = false;
}

void RayTracedSPH::OnMouseMove(float posX, float posY)
{
	if (m_tracking)
	{
...
void RayTracedSPH::OnMouseWheel(float deltaZ, float posX, float posY)
{
	const auto focusPt = XMLoadFloat3(&m_focusPt);
	auto eyePt = XMLoadFloat3(&m_eyePt);
...
void RayTracedSPH::OnMouseLeave()
{
	m_tracking = false;
}

void RayTracedSPH::ParseCommandLineArgs(wchar_t* argv[], int argc)
{
...
void RayTracedSPH::PopulateCommandList()
{
...
void RayTracedSPH::MoveToNextFrame()
{
	// Schedule a Signal command in the queue.
...
void RayTracedSPH::SaveImage(char const* fileName, Buffer* pImageBuffer, uint32_t w, uint32_t h, uint32_t rowPitch, uint8_t comp)
{
...
void RayTracedSPH::EnableDirectXRaytracing(IDXGIAdapter1* adapter)
{
